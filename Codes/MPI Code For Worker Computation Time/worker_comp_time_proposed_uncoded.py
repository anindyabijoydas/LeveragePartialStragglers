"""
Finding the worker computation time for the proposed uncoded scheme
Having two random matrices A and B
We have n workers and s stragglers.
Storage fraction gammaA = a1/a2 and gammaB = b1/b2.
Matrices A and B are partitioned into a2 and b2 block columns.
ellA and ellB are the number of uncoded blocks for A and B, respectively.
"""
...
if rank == 0:
    ...
    for k in range(0,n):
        resA = {};
        resB = {};
        vecA = np.ones([1,betaA],dtype= float)
        vecB = np.ones([1,betaB],dtype= float)  
    ...
    for i in range (0,n):
        print("Computation time (using uncoded approach) for processor %s is %s" %(i,computation_time[i]))
